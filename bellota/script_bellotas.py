...
def deleteYcomponentandCrCb(img):
    ...
    for row in img:
        ...
        for pixel in row:
            ...
            CrCb = abs(int(pixel[2]) - int(pixel[1]))
...
def getStatistics(filename,image,num_clases,imageLabelClass):
    ...
    for nclass in range(num_clases):            
        ...
        for i in range (filas*columnas):
            if imageLabelClass[i] == nclass:
                ...
                #diferencia = abs(pixel[2] - pixel[1])
                diferencia = abs(int(pixel[2]) - int(pixel[1]))
                #print pixel[2],"    ",pixel[1],"    ",diferencia
    ...
    for num in range(len(listMeanCrCb)):
        print filename[1]+','+str(num+1)+","+str(listMeanCrCb[num])+","+str(listMinCrCb[num])+","+str(listMaxCrCb[num])+","+str(listDesvCrCb[num]) + "\n"
    #for num in range(len(listMeanCrCb)):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        for directory in os.listdir(sys.argv[1]):
            if os.path.isdir(directory) and "foto" in directory:
                    ...
                    if clasesPossibles:
                        for nclass in clasesPossibles:
                            ...
                            for contour in contours:
                                ...
                                i += 1
                            cv2.imwrite('exportImages/class_'+str(nclass+1)+"___"+photo.split(".")[0]+".png", imageOutput)
